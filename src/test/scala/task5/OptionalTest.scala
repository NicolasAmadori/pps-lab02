...
class OptionalTest:
  ...
  @Test def filterShouldReturnEmptyWhenNonEmptyButPredicateNotSatisfied(): Unit =
    val nonEmpty = Optional.Maybe(0)
    val result = Optional.filter(nonEmpty, _ > 2)
    assertTrue(Optional.isEmpty(result))

  @Test def filterShouldReturnEmptyWhenNonEmptyAndPredicateSatisfied(): Unit =
    val nonEmpty = Optional.Maybe(5)
    val result = Optional.filter(nonEmpty, _ > 2)
    assertEquals(5, Optional.orElse(result, 0))
